...
pte_t* walk(pagetable_t pgtbl, uint64 va, int alloc) {
    ...
    for(int level = 2; level > 0; level--) {
        ...
        if((*pte) & PTE_V) { // 如果是valid的话
            pgtbl = (pagetable_t)(PTE2PA(*pte) + PA2VA_OFFSET); // pte右移10位左移12位得到下一level的pagetable的地址
        } else {
            if(alloc == 0 || (pgtbl = (pde_t*)kalloc()) == 0) { //给pagetable分配内存
                return 0;
            }
            // printk("pgtabl: %lx\n", pgtbl);
            memset(pgtbl, 0x0, PGSIZE);
            *pte = ((unsigned long)(*(pte)) & 0xffc0000000000000) | PA2PTE(((uint64)pgtbl - PA2VA_OFFSET)) | PTE_V;
            // printk("alloc pte: %lx %lx %lx \n", pte, *pte, PA2PTE(((uint64)pgtbl - PA2VA_OFFSET)));
        }
    }
...
void create_mapping(pagetable_t pgtbl, uint64 va, uint64 pa, uint64 sz, int perm) {
   ...
   printk("create: %lx %lx %lx\n", va, pa, sz);
    ...
    for (; va < end_va; va += PGSIZE, pa += PGSIZE) {
        ...
        *pte = ((unsigned long)(*(pte)) & 0xffc0000000000000) | (uint64)(PA2PTE(pa) | perm | PTE_V);
        // printk("pte: %lx pte: %lx pa: %lx\n", pte, *pte, pa);
    }
}
...
void setup_vm_final(void) {
    ...
    create_mapping(swapper_pg_dir, (uint64)_stext, (uint64)_stext - PA2VA_OFFSET, (uint64)_etext - (uint64)_stext, 
                PTE_X | PTE_R | PTE_V);
    

    // mapping kernel rodata -|-|R|V
    printk("create mapping for rodata...\n");
    create_mapping(swapper_pg_dir, (uint64)_srodata, (uint64)_srodata - PA2VA_OFFSET, (uint64)_erodata - (uint64)_srodata, 
                PTE_R | PTE_V);

    // // mapping other memory -|W|R|V
    printk("create mapping for data...\n");
    create_mapping(swapper_pg_dir, (uint64)_sdata , (uint64)_sdata - PA2VA_OFFSET, PHY_END + PA2VA_OFFSET - (uint64)_sdata,
                    PTE_W | PTE_R | PTE_V);

    // set satp with swapper_pg_dir
    uint64 satp_val = (uint64)(((uint64)0x1000 << 51 | 0 | ((uint64)((uint64)swapper_pg_dir - PA2VA_OFFSET) >> (uint64)12))); // mode | asid | PPN
